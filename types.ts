
export interface User {
  id: string;
  walletAddress: string;
  username: string;
  avatarUrl: string;
  coverUrl?: string; // Profile cover image URL
  email?: string;
  bio?: string;
  balance: number; // in USDT (Game Currency)
  totalHands?: number; // Added for VIP tracking
  friends?: string[]; // List of Friend User IDs
  preferences: {
    showWinRate: boolean;
    showPnL: boolean;
    incognito?: boolean; // New: Mask username on tables
    hideBalance?: boolean; // New: Hide balance on public profile
    allowFriendRequests?: boolean; // New: Social privacy
  };
  isVerified?: boolean;
  // Ecosystem Fields
  referralCode?: string;
  referralRank?: 'FREE' | 'AGENT' | 'BROKER' | 'PARTNER' | 'MASTER'; // Hybrid Override Model v1.1
  ecosystemStats?: {
    totalReferrals: number; // Total network size
    directReferrals: number; // Level 1 connections
    networkHands: number; // Total hands played by network
    referralEarnings: number;
    pendingRewards: number;
    // For Rank Tracking
    directAgents: number; // How many directs are Agents
    directBrokers: number; // How many directs are Brokers
    directPartners: number; // How many directs are Partners
  };
}

export enum GameType {
  CASH = 'CASH',
  FUN = 'FUN'
}

export enum Speed {
  REGULAR = 'REGULAR',
  TURBO = 'TURBO',
  HYPER = 'HYPER'
}

export interface PokerTable {
  id: string;
  creatorId?: string; // Table creator (for analytics only)
  name: string;
  smallBlind: number;
  bigBlind: number;
  seats: 6 | 9;
  occupiedSeats: number;
  buyInMin: number;
  buyInMax: number;
  speed: Speed;
  rakeCap: number; // Max rake in USDT per hand
  type?: GameType;
  isPrivate?: boolean;
  password?: string;
}

export interface GameStats {
  totalHands: number;
  winRate: number; // bb/100
  totalWinnings: number;
  roi: number;
  bestHand: string;
}
